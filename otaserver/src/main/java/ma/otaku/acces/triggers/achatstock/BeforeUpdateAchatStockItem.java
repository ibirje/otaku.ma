...
@Mode(Mode.BEFORE)
public class BeforeUpdateAchatStockItem extends Trigger<AchatStockItemDB>{

	@Override
	public void execute(AchatStockItemDB old, AchatStockItemDB nnew) throws Exception {
	
		// maybe move validation ici ???

		if(nnew.getCoutTotal() == null || nnew.getCoutTotal() < 1)
	...
	private void calculeCoutTotal(AchatStockItemDB nnew) {
		...
		Double coutTotal = nnew.getPrixUnite() + (nnew.getPrixUnite() / achat.getPrixTotal()) * achat.getFraisSupplementaires();
